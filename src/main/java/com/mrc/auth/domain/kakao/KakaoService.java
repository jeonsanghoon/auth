...
import com.mrc.auth.advice.exception.CommunicationException;
import com.mrc.auth.domain.kakao.dto.KakaoProfileDTO;
import com.mrc.auth.domain.kakao.dto.RetKakaoAuthDTO;
...
public class KakaoService {
    ...
    public RetKakaoAuthDTO getKakaoTokenInfo(String code) {
        // Set header : Content-type: application/x-www-form-urlencoded
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_FORM_URLENCODED);
        // Set parameter
        ...
        if (response.getStatusCode() == HttpStatus.OK) {
            return gson.fromJson(response.getBody(), RetKakaoAuthDTO.class);
        }
        return null;
    }
}